...
- **后台管理**：积分手动增减（含重复登记检测）、删除/修改密码保护（通过环境变量 `DELETE_PASSWORD` 配置，系统永不显示默认密码）、删除留痕日志、登记人名单维护、SQLite/Excel 双方式数据导出
...
| `DELETE_PASSWORD` | 删除/修改记录密码（部署时务必自定义，系统永不显示默认值） | 部署时设置 |
...
- **删除/修改保护**：任何记录的删除或修改操作必须输入密码（通过环境变量 `DELETE_PASSWORD` 配置），并记录删除日志（含执行门店、时间，永久保留）。系统任何位置均不显示默认密码明文。
